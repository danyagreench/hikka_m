...
class InlineBossList(loader.Module):
    ...
    async def helpibl(self, message):
        ''' - помощь по модулю'''
        await utils.answer(message,f"<b><emoji document_id=5787544344906959608>ℹ️</emoji> Помощь по модулю InlineBossList</b>\n\n<emoji document_id=5787543653417225686>⏺</emoji> <b>Если у вас перестал работать Инлайн Босс Лист, и показывает ошибку, просит посмотреть логи, зайдите в группу в архиве, которую создал модуль «InlineBossList», и удалите там сообщение последнее, которое было от вас, скорее всего перестало работать только из-за того что бот MineEVO не скинул босс лист.</b>")
